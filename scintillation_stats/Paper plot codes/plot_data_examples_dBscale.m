...
ReceivedTimeSeries = BandPassFilter(input_series(:, 1), 0, 600, 8000); 
...
s3 = subplot(2, 2, 3); 
plot(linspace(0, 8000, size(Sf1,2)), 20*log10(abs(Sf1)/sqrt(2)), 'k');
setFont(18);
...
xlabel('Frequency (Hz)'); 
ylabel('Normalized spectrum (dB re \muPa)');

s4 = subplot(2, 2, 4); 
T = size(input_series, 1)/f_samp; 
ReceivedSpectrum = ifft(input_series(:, 1)/(10^(220/20)))*T; 
plot(linspace(0, 8000, size(ReceivedSpectrum, 1)), 20*log10(abs(ReceivedSpectrum)), 'k'); 
setFont(18); 
xlim([350 500])
ylim([-140 -80])
set(gca, 'ytick', [-140:10:-80])
xlabel('Frequency (Hz)'); 
